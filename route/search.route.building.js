	...
	global.TYPESENSECOLLECTIONNAME = getName.eventValue
        ...
        if (DecimalSearch) {
           var searchParameters = {
             ...
             query_by:
               "Name, Month, OptionTypeString,StrikePriceString, Description",
             per_page: 250,
           ...
           var searchParameters = {
             q: req.params.term.toLowerCase(),
             query_by:
               "Name, Month,DisplayName, OptionTypeString,StrikePriceString, Description",
             per_page: 250,
             sort_by: "Series:asc",
           };
         } 
...
router.route("/searchbyid").post(async (req, res) => {
  ...
  if (req.body.instruments.length <= 0) {
    return res.send({
      status: "error",
      msg: "instruments should have more than one object of segment & instrumentId",
    });
  }
