...
const MailNotify =(Description)=>{
  Mail.sendMail(
        "pradeep.g@tabtree.in","amarnath.s@tabtree.in","madhusudhan.p@tabtree.in", "New Task Assigned","Dear " + "Dev Team" + ",<br><br>  You have been Assigned a Task. Appreciate your Quick Response.<br>" + "Task ID: " + "1" + "<br>Assigned  by:" + "RyzLiveServer" + "<br>Description: " + Description + "<br>To be Completed by :" + moment().format("DD MMM YYYY hh:mm A") + "<br>Task Name : " + "Task Completed" + "<br><br>Thanking You,<br><br> Team Treeone."
    )
}
var LOG_PREFIX =
    new Date().getDate() +
  ...
  try {
    ...
    try {
      ...
      for (let mk = 0; mk < totalSegment.length; mk++) {
        ...
        if(StrikePrice) {
          StrikePriceString = ''+Number(StrikePrice).toFixed(2)+''
        } else {
          StrikePriceString = ''
        }
      ...
      checkCountValue=multipleArray.length
    ...
    MailNotify(`Collection Name - ${neworexistingCollection} Typesense Cronjob Completed`)
    const TypeNoti=await client.collections(neworexistingCollection).retrieve()
    console.log(`Gogdev Cron ${LOG_PREFIX} Collection Count ${String(TypeNoti.num_documents)} Fetch from Api ${checkCountValue}`)
  }catch (error) {
    console.log(error);
...
//(async () => {
//    await NotiFunc("Server Restarted",`Gogdev Cron ${LOG_PREFIX} Self Calling Function Called`)
//    const wait = await client.collections().retrieve()
//    await wait.map((data)=>console.log(`******All Collection Name ${data.name}*******`))
...
//    console.log("getName",getName.eventValue);
//    if(getName) {
//      try {
//          const resp = await client.collections(getName.eventValue).retrieve()
//	  if(resp){
//            console.log("collection found...");
//          }
...
(async()=>{
  //SearchEngineLatestUpdater()
  //MailNotify(`Collection Name: ${neworexistingCollection} <br>Typesense CronJob Completed`) 
  // Message("Gogdev Restarted..",`Gogdev ${LOG_PREFIX}`)
  console.log("Server Time",`${LOG_PREFIX}`)
})()
new CronJob('00 09 * * *', SearchEngineLatestUpdater, null, true,'Asia/Kolkata');
