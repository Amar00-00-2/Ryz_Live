...
module.exports = (io) => {
  ...
  ioconnection.on('connect', async (socket) => {
    ...
    socket.on('run_marketdata', async (data, userSessionData) => {
      ...
      const getInstrument = await subscriptionTrackingModel.findOne({ where: { user_id: userId }, order: [['id', 'DESC']], })
      ...
      await subscriptionTrackingModel.create({ user_id: userId, subsscription: JSON.stringify([...instruments]) })
      ...
      xtsMarketDataWS.onMarketDepthEvent(marketDepthData => {
        // console.log('searchmarketdataId', marketDepthData.ExchangeInstrumentID)
